...
time_series_IR <- list(Alaska = time_series %>% filter(state=="AK") %>% select(date,positiveTests))
time_series_IR$California <- time_series %>% filter(state=="CA") %>% select(date,positiveTests) 
time_series_IR$New_Jersey <- time_series %>% filter(state=="NJ") %>% select(date,positiveTests) 
time_series_IR$Tennessee <- time_series %>% filter(state=="TN") %>% select(date,positiveTests)
time_series_IR$District_of_Columbia <- time_series %>% filter(state=="DC") %>% select(date,positiveTests)


data_to_plot2 <- tibble(Date_Alaska = time_series_IR$Alaska$date[seq(1,1300,by=100)],
                        IR_Alaska = time_series_IR$Alaska$positiveTests[seq(1,1300,by=100)],
                       Date_California = time_series_IR$California$date[seq(1,1300,by=100)],
                       IR_California = time_series_IR$California$positiveTests[seq(1,1300,by=100)],
                       Date_New_Jersey = time_series_IR$New_Jersey$date[seq(1,1300,by=100)],
                       IR_New_Jersey = time_series_IR$New_Jersey$positiveTests[seq(1,1300,by=100)],
                       Date_Tennessee = time_series_IR$Tennessee$date[seq(1,1300,by=100)],
                       IR_Tennessee = time_series_IR$Tennessee$positiveTests[seq(1,1300,by=100)],
                       Date_District_of_Columbia = time_series_IR$District_of_Columbia$date[seq(1,1300, by=100)],
                       IR_District_of_Columbia = time_series_IR$District_of_Columbia$positiveTests[seq(1,1300, by=100)])
...
library(cowplot)
fig1<- ggplot(data_to_plot, aes(x=Date_Alaska,y=IR_Alaska)) +
 geom_point() + labs(x="Date",y="Infection Rate", title="Alaska") + theme_bw()
 fig2<- ggplot(data_to_plot, aes(x=Date_California,y=IR_California)) +
 geom_point() + labs(x="Date",y="Infection Rate", title="California") + theme_bw()
 fig3<- ggplot(data_to_plot, aes(x=Date_New_Jersey,y=IR_New_Jersey)) +
 geom_point() + labs(x="Date",y="Infection Rate", title="New Jersey") + theme_bw()
 fig4<- ggplot(data_to_plot, aes(x=Date_Tennessee,y=IR_Tennessee)) +
 geom_point() + labs(x="Date",y="Infection Rate", title="Tennessee") + theme_bw()
 fig5<- ggplot(data_to_plot, aes(x=Date_District_of_Columbia,y=IR_District_of_Columbia)) +
 geom_point() + labs(x="Date",y="Infection Rate", title="District of Columbia") + theme_bw()
 plot_grid(fig1 + theme(legend.justification = c(0,1)),
           fig2 + theme(legend.justification = c(1,0)),
           fig3 + theme(legend.justification = c(0,1)),
           fig4 + theme(legend.justification = c(1,0)),
